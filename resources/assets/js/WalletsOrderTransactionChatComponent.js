...
export default {
    ...
    methods: {
        ...
        sendBankAccount() {
            ...
            this.loader = true;
            axios.post(window.location.origin + '/wallets-send-bank-account/' + this.order.id, {
        ...
        sendMessage(event) {
            ...
            if (!event.shiftKey) {
                ...
                if (controlMessage.length > 0 || this.file !== '') {
                    ...
                    axios.post(
                        window.location.origin + '/api/wallets/create-order-message/' + this.order.id,
                        formData,
                        {
                            params: {
        ...
        subscribe() {
            ...
            pusher.subscribe('wallets-operator-' + this.user.id + '-channel');
            pusher.bind(
        ...
        getAccountStatus() {
            ...
            axios.get(window.location.origin + '/api/wallets/bank-accounts-status/' + this.order.id)
                .then(re => {
        ...
        cancelBankAccount(id) {
            //console.log(id);
            this.loader = true;
            axios.post(window.location.origin + '/wallets-cancel-bank-account/', {
                account_id: id,
                _token: $('meta[name="csrf-token"]').attr("content")
            }).then(re => {
        ...
        reclaim() { //Deprecated
            axios.get(window.location.origin + '/api/wallets/reclaim/' + this.order.id) //Doesn't exist
                .then(re => {
                    this.attended    = 1;
