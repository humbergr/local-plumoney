...
export default {
    ...
    data() {
        return {
            ...
            formUrl: this.getUrl('/make-purchase?paymentMethod=cash'),
            ...
            loadingSend: false,
            user_first_name: JSON.parse(this.user)['first_name'],
            user_last_name: JSON.parse(this.user)['last_name'],
            user_email: JSON.parse(this.user)['email'],
        };
    },
    onIdle() {
        if (this.alertForIdle === null) {
            this.alertForIdle = $.alert({
                title: '¡Alerta!',
                content: 'Tienes 4 minutos de inactividad. <br> En 60 segundos cerraremos tu sesión por seguridad.',
                type: 'red'
            });
    ...
    methods: {
            ...
            axios.get(window.location.origin + '/get-price', {
                ...
                .then(
                    re => {
                        ...
                        let noViable = $('#sm-origen option[value^="' + localStorage.getItem('sender_country') + '"]')
                            .attr('data-nb') ? $('#sm-origen option[value^="' + localStorage.getItem('sender_country') + '"]')
                            .attr('data-nb') : null;
        ...
        gettingAccounts() {
            this.loader = true;
            axios.get(window.location.origin + '/api/getting-my-accounts?currency=' + this.receiver
            )
                .then(re => {
        ...
        getFromLocalstorage() {
            ...
            let flag               = $('#sm-origen option[value^="' + localStorage.getItem('sender_country') + '"]')
                    .attr('data-flag'),
                rFlag              = $('#sm-destino option[value^="' + localStorage.getItem('receiver_country') + '"]')
                    .attr('data-flag');
        ...
        onPaymentMethodUpdate(paymentMethod) {
            ...
            if (paymentMethod !== 'cash' && !Array.isArray(paymentMethod)) {
                this.payment_total -= this.fees;
                this.fees    = 0;
                this.formUrl = this.getUrl('/make-purchase?paymentMethod=' + paymentMethod);
            }
        },
    ...
    computed: {
        ...
        totalWithBonus: function () {
            let total = parseFloat(this.toReceive.replace(/,/g, "")) + parseFloat(this.formated_bonus.replace(",", ""));
