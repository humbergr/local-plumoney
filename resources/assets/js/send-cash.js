...
export default {
    props: ['order', 'user', 'profile', 'destination', 'accounts'],
    ...
    methods: {
        ...
        sendMessage() {
            if (this.message !== '') {
                ...
                axios.post(window.location.origin + '/api/create-order-message/' + this.order.id, {
        ...
        sendFile(ID, index) {
            ...
            if (fileToUpload) {
                ...
                axios.post(window.location.origin + '/api/create-order-message/' + this.order.id, formData, {
                    params: {
                        bankAccountID: ID,
                        message: '',
                        _token: $('meta[name="csrf-token"]').attr("content")
        ...
        getQueue() {
            axios.get(window.location.origin + '/api/get-queue/' + this.order.id)
                .then(re => {
                    this.queue_number = re.data;
                })
        },
        getBankAccounts() {
            axios.get(window.location.origin + '/api/bank-accounts/' + this.order.id)
                .then(re => {
        ...
        subscribe() {
            ...
            pusher.subscribe('transactions-channel');
